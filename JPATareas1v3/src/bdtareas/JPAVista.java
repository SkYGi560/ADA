...
public class JPAVista {
    ...
    public static void verTareasNamedQuery(TareasJpaController dao, Integer numLista) {
        System.out.println();
        System.out.println("Mostrando todas las tareas (NamedQuery):");

        List<Tareas> lista = null;
        switch(numLista){
            case 1 -> lista = findTareasNamedQuery(dao);
            case 2 -> lista = verTareasProximosDias(dao);
        }
        System.out.println("+----+------------+------------------------------------------+------------+");
