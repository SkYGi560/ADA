...
public class JPAVista {
    ...
    public static List<Tareas> verTareasProximosDias(TareasJpaController dao) {
        ...
        try (EntityManager em = dao.getEntityManager()) {
            TypedQuery<Tareas> query = em.createQuery("SELECT t FROM Tareas t WHERE t.fecha BETWEEN :desde AND :hasta", Tareas.class);
            query.setParameter("desde", fechaDesde.getTime());
            query.setParameter("hasta", fechaHasta.getTime());
            return query.getResultList();
        }
    }

    public static List<Tareas> verTareasDescripcionLike(TareasJpaController dao) {
        ...
        System.out.println("==============================");
        try (EntityManager em = dao.getEntityManager()) {
            TypedQuery<Tareas> query = em.createQuery("SELECT t FROM Tareas t WHERE t.descripcion LIKE :entrada", Tareas.class);
            query.setParameter("entrada", entrada);
            return query.getResultList();
        }
    }

    public static void verTareas(TareasJpaController dao, Integer numLista) {
        System.out.println();
        ...
        switch (numLista) {
            case 1 ->
                lista = findTareasNamedQuery(dao);  
            case 2 ->
                lista = verTareasProximosDias(dao);
            case 3 ->
                lista = verTareasDescripcionLike(dao);
        }
        System.out.println("+----+------------+------------------------------------------+------------+");
