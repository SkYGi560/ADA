...
public class ServiceRESTMath {
    ...
    @Produces(MediaType.TEXT_PLAIN)
    public int restar(@PathParam("operando1") int op1,
            @PathParam("operando2") int op2) {
        return op1 - op2;
    }
    ...
    @Produces(MediaType.TEXT_PLAIN)
    public int multiplicar(@PathParam("operando1") int op1,
            @PathParam("operando2") int op2) {
        return op1 * op2;
    }
    ...
    @GET
    @Path("/porcentaje/{operando1}/{operando2}")
    @Produces(MediaType.TEXT_PLAIN)
    public double porcentaje(@PathParam("operando1") double op1,
            @PathParam("operando2") double op2,
            @QueryParam("decimales") int decimales) {
        double resultado = (op1 / 100) * op2;
        resultado = Math.round(resultado * Math.pow(10, decimales))
                / Math.pow(10, decimales);
        return resultado;
    }
